...
void recursion(node *head,  int level, bool leftToRight) {
	if(head == NULL) { return; }	

	if(level == 1) cout << head->data << " " ;

	else {
		if(level > 1 && leftToRight ) {
			recursion(head->left, level-1, leftToRight);
			recursion(head->right, level-1, leftToRight);
		}
		if(level > 1 && !leftToRight ) {
			recursion(head->right, level-1, leftToRight);
			recursion(head->left, level-1, leftToRight);
...
int main() {
	...
	for(int i =1 ; i <= h; i++) {
		cout << "Level " << i << "  :: " ;
		recursion(head, i, leftToRight);
		cout << endl;
		if(i % 2 == 1) leftToRight = false;

...
int main() {
	int gasAtStation[] = {1,2,3,4,5};
	int gasRequired[] ={3,4,5,1,2};

	int sumGas=0, sumCost=0, tank=0, start=0, startIndex=0, end=2;
	recursion(gasAtStation, gasRequired, start, end, startIndex, sumGas, sumCost, tank);

	if(sumGas > sumCost) cout << "Min cost Index " << startIndex+1 << endl;

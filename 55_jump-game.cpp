...
int main()
{
    int arr[] = {3,2,1,0,4}; // { 1, 3, 6, 1, 0, 9 };
    int size = sizeof(arr) / sizeof(int);
    if(minJumps(arr,size) == 2147483647) cout << "Not possible to reach the end" << endl;
    else cout << "Minimum number of jumps to reach"
         << " end is " << minJumps(arr, size) << endl;
    return 0;
}
